...
<body class="bg-gray-100 min-h-screen font-sans antialiased">
    <div id="app" class="flex flex-col min-h-screen">
        <!-- Mensajes Flash -->
        @if(session('success'))
            <div v-if="show" class="fixed bottom-4 right-4 bg-green-500 text-white p-4 rounded-lg shadow-lg z-50 transition-transform transform ease-in-out duration-300">
                {{ session('success') }}
        ...
        @if(session('error'))
            <div v-if="show" class="fixed bottom-4 right-4 bg-red-500 text-white p-4 rounded-lg shadow-lg z-50 transition-transform transform ease-in-out duration-300">
                {{ session('error') }}
                <button @click="show = false" class="ml-2 text-white">&times;</button>
            </div>
        @endif
                ...
                <div class="flex justify-between h-16">
                    ...
                    <div class="-mr-2 flex items-center sm:hidden">
                        <button @click="open = !open" class="inline-flex items-center justify-center p-2 rounded-md text-gray-400 hover:text-gray-500 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-blue-500">
                            ...
                            <svg class="h-6 w-6" v-if="!open" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
